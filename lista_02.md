...
var idade = prompt('Insira sua idade aqui:');

se (idade < 16) {
    imprima("Não pode votar");
} senãoSe (idade >= 16 && idade < 18) {
...
Classe Retangulo extendidaDe FormaGeometrica {
	Metodo Construtor (cor, altura, largura) {
		Super (cor);
		essa altura = altura;
		essa largura = largura;    
		}
...
Classe Circulo extendidaDe FormaGeometrica {
	Metodo Constructor (cor, raio, pi){
		Super (cor);
		esse raio = raio
		esse pi = 3.14
		}
...
let areaCirculo = novo Circulo(amarelo, 2);
 
let areaRetangulo = novo Retangulo(vermelho, 5, 4);
...
variavel posicaoInicial = 0 

variavel velocidade inicial = 0 

variavel tempo inicial = 0 

variavel extensaoDaPista = 10

variavel aceleracao = 2



funcao corrida (posicao, velocidade, tempo) { 

paraQuando (tempo = 0 ; tempo ≤ tempoFinal; tempo += 1 ) { 
...
se (posicao ≤ extensaoDaPista) {

    posicao = posicaoInicial + velocidade * tempo

}senão {

imprima (”corrida acabou”) }
